...
import { Search, Calendar, Paperclip } from "lucide-react";
...
const TaskCard = ({ task, onClick, provided, innerRef }) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className="bg-white rounded-lg p-4 shadow-sm border border-gray-200 hover:shadow-md hover:border-blue-300 transition-all cursor-pointer select-none mb-3"
    >
...
const MyTasks = () => {
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      const response = await axiosInstance.patch(`/api/tasks/${draggableId}/status`, {
        status: newStatus
      });
      setData((prev) => ({
        ...prev,
        tasks: prev.tasks.map((t) => (t._id === draggableId ? response.data : t))
      }));
    } catch (error) {
      console.error(error);
    }
            ...
            return (
              <div key={col.status} className="bg-gray-50 rounded-xl p-4 flex flex-col min-h-[500px]">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-semibold text-gray-770 uppercase text-xs tracking-wide">
                    {col.label}
                  </h3>
                ...
                <Droppable droppableId={col.status}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`flex-1 max-h-[calc(100vh-300px)] overflow-y-auto space-y-3 pr-1 transition-colors duration-200 rounded-lg ${
